...
function DashboardContent() {
  ...
  useEffect(() => {
    if (messages.length > 0 && currentChatId && recipe) {
      ...
      // @ts-ignore - Null is acceptable runtime data even if types differ temporarily
    ...
    try {
      ...
      if (response.status === "success" && response.data) {
        ...
        await createNewChat({
          ...
          food_quality: response.data.food_quality || undefined,
        });
...
export default function DashboardPage() { return <Suspense fallback={<div>Loading...</div>}><DashboardContent /></Suspense>; }